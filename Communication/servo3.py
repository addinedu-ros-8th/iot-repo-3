...
# Arduino 연결 시도 (예시: /dev/ttyACM3, /dev/ttyACM0)
ser0 = None
ser1 = None
try:
...
def run_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        try:
            ...
            print("서버 소켓 accept 에러:", e)
            return
        with conn:
            print("클라이언트 연결됨:", addr)
            conn.settimeout(0.5)
            # 두 Arduino 리스너 스레드 생성 (ser0, ser1)
            if ser0:
...
class MainWindow(QWidget):
    ...
    def closeEvent(self, event):
        global exit_flag
        exit_flag = True  # 창 종료 시 서버 스레드 종료 요청
        event.accept()

if __name__ == "__main__":
    # 서버 스레드 실행 (데몬 스레드로 실행하여 메인 종료 시 함께 종료됨)
    server_thread = threading.Thread(target=run_server, daemon=True)
    server_thread.start()

    # 필요시 키보드 리스너를 추가할 수 있음 (현재는 GUI 종료로 제어)
